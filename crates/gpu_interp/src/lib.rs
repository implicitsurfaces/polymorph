...
pub fn create_and_fill_buffers(
    ...
    let bytecode_buffer = {
        ...
        let tape_bytes = expression.tape_bytes();
    ...
    let offsets_buffer = {
        ...
        contents[..expression.offsets.len()].copy_from_slice(&expression.offsets);
        device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
    ...
    let pc_max_buffer = {
        let mut contents = vec![0u32; MAX_TILE_COUNT as usize];
        contents[..expression.lengths.len()].copy_from_slice(&expression.lengths);
        device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
