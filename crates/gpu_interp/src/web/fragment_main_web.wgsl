...
fn fragment_main_web(@builtin(position) pos: vec4<f32>) -> @location(0) vec4<f32> {
  ...
  let pixel_value = pixel_group[x % 4u];

  let size = f32(viewport[0]);

  // max visible distance is +/- viewport size (it's square)
  let pmax = size / 2.0;
  ...
  // draw contour lines
  let band_spacing = 30.0;
  let distance_to_closest_band = min(abs(pixel_value)                 % band_spacing,
                                    (abs(pixel_value) + band_spacing) % band_spacing);
  if (distance_to_closest_band < 1) {
