...
def parse_style_attr(style_str: str) -> Dict[str, str]:
    """
    Parse SVG style attribute string into a dictionary.
...
def dict_to_style_str(style_dict: Dict[str, str]) -> str:
    """
    Convert a style dictionary back to a style string.
...
def remove_ids(element: Union[ET.Element, ET.ElementTree],
...
def remove_style_properties(element: Union[ET.Element, ET.ElementTree], 
                            properties: Dict[str, str],
                            recursive: bool = True) -> None:
    """
    Remove style properties from an SVG element's style attribute.
    
    Args:
        element: ElementTree or Element to modify
        properties: The key/value pairs of properties to remove
        recursive: Whether to process child elements recursively
    """
    # If we're passed an ElementTree, get its root
    if isinstance(element, ET.ElementTree):
    ...
    if style:
        ...
        style_dict = parse_style_attr(style)

        # Remove undesired key/value properties
        for property_name, property_value in properties.items():
            if property_name in style_dict:
                if property_value is None or style_dict[property_name] == property_value:
        ...
        if ('stroke' not in style_dict or style_dict['stroke'] == 'none'):
            for property_name in [
                ...
                'stroke-opacity', 'stroke-dashoffset', 'stroke-miterlimit']:
                if property_name in style_dict:
                    del style_dict[property_name]
...
def remove_namespaced_attributes(element: Union[ET.Element, ET.ElementTree]) -> None:
    """
    ...
    namespaced_attrs = [
        attr for attr in element.attrib.keys()
...
def split_svg_group(input_file: str, group_name: str, output_dir: str):
    """
    Split elements from a specified group in an SVG file into separate SVG files.
    
    Args:
        input_file (str): Path to the input SVG file
        group_name (str): Name of the group to split (without '#' or '.')
    ...
    for icon in group:
        ...
        remove_style_properties(new_tree, {
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate a separate SVG icon file for each icon of a given SVG group')
    parser.add_argument('input_file', help='Input SVG file path')
    parser.add_argument('group_name', help='Name of the group to split into separate icon files')
