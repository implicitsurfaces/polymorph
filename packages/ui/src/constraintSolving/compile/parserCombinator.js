...
const makeTest = (
  ...
  rule instanceof RegExp
    ...
    ? (x) => rule.startsWith(x)
    : Array.isArray(rule)
    ? (x) => rule.map(makeTest).some((f) => f(x))
    : rule; // is function
...
const convert = (pred) => (s) => {
  return s[0] && s[0].type === pred ? [s[0], s.slice(1)] : null;
};
...
const star =
  ...
  (s) => {
    ...
    return arr.length > 0
      ? [
          transform ? transform(arr.map(([x]) => x)) : arr.map(([x]) => x),
          arr[arr.length - 1][1],
        ]
      : [[], s];
  };
...
const or =
  (preds, transform = null) =>
  (s) => {
    const result = preds.reduce(
      (acc, cur) => acc || (typeof cur === "string" ? convert(cur) : cur)(s),
      false,
    );
...
const convertBi = (x) => {
  if (
    x[2].type === "binary" &&
    PRECEDENCE[x[2].operator] <= PRECEDENCE[x[1].value]
  ) {
    return convertBi([
...
const prog = (s) => or([bi, exp, "symbol", "number"])(s);

const convertArgs = (x) => (x[0] === null ? [x[1]] : [...x[0], x[1]]);
const args = (s) =>
  and(
    [opt(plus(and([prog, ","]), (x) => x.map((y) => y[0]))), prog],
    convertArgs,
  )(s);
...
const bi = (s) => and([exp, or(OPERATORS.flat()), prog], convertBi)(s);
